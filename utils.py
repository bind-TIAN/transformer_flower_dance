...
def generate_ds(data_root: str,
    ...
    AUTOTUNE = tf.data.experimental.AUTOTUNE  # 由tensorflow自动进行并行化的操作

    def process_train_info(img_path, label):
        image = tf.io.read_file(img_path)  # 读取图片
        image = tf.image.decode_jpeg(image, channels=3)  # 这两条语句搭配使用用来读取图片
        image = tf.cast(image, tf.float32)  # 将image的数据格式转化成float类型
        image = tf.image.resize_with_crop_or_pad(image, train_im_height, train_im_width)  # 裁剪和填充图像，变成指定宽度和高度
        image = tf.image.random_flip_left_right(image)  # 随机翻转图像
        image = (image / 255. - 0.5) / 0.5  # 将图像调整成[0-1]区间
        return image, label

    def process_val_info(img_path, label):  # 这部分操作同上，这里就不再赘述
        image = tf.io.read_file(img_path)
        image = tf.image.decode_jpeg(image, channels=3)
        image = tf.cast(image, tf.float32)
        image = tf.image.resize_with_crop_or_pad(image, val_im_height, val_im_width)
        image = (image / 255. - 0.5) / 0.5  # 与训练数据不同的是，这里不需要对图像进行翻转操作了
        return image, label
    ...
    train_ds = tf.data.Dataset.from_tensor_slices((tf.constant(train_img_path),
                                                   tf.constant(train_img_label)))  # 对训练数据进行特征切片，每组数据对于一个标签
    ...
    val_ds = tf.data.Dataset.from_tensor_slices((tf.constant(val_img_path),
                                                 tf.constant(val_img_label)))  # 对测试数据进行特征切片，每组数据对于一个标签
